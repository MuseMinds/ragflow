...
def _vision_model_config(tenant_id, parser_config):
    pinned_id = parser_config.get("img2txt_id")
    if pinned_id:
...
def _fail_closed_vision_path(parser_config, vision_model=None):
    return parser_config.get("img2txt_id") == MUSEMIND_GEMINI_IMAGE_ID or getattr(getattr(vision_model, "mdl", None), "content_free_observability", False) is True
...
def chunk(filename, binary, tenant_id, lang, callback=None, **kwargs):
    ...
    if any(filename.lower().endswith(ext) for ext in VIDEO_EXTS):
        try:
            ...
            cv_model_config = _vision_model_config(tenant_id, parser_config)
            ...
            if _fail_closed_vision_path(parser_config, locals().get("cv_mdl")):
